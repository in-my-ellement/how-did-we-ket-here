...
def calc(file_name, num_qubits = 48):
    ...
    qc = qtn.CircuitMPS.from_gates(gates, num_qubits, progbar=True, max_bond=4, cutoff=1e-20)
    prob = np.array([0] * num_qubits)
    for b in qc.sample(100, seed=43): 
        prob += np.array([int(n) for n in list(b)])

    return "".join(str(round(n)) for n in prob / 100)

if __name__ == "__main__":
    calc("P1_little_peak.qasm")